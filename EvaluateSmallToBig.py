...
#    plt.plot(x, y_pred, color = "g")
    ...
    # putting labels
#    plt.xlabel('Time Saving Percentage (Small MPS)')
    plt.xlabel('Execution Time of Baseline 61-Atom Model (seconds)')
    plt.ylabel('Execution Time of 136-Atom Model (seconds)')
#    plt.ylabel('Time Saving Percentage(Larger MPS)')
#    plt.plot(x,y_pred,color='r',linestyle="--",marker="*",linewidth=1.0,label="Linear Regression")
...
#        i=int(random.uniform(0,len(eleName)-1))
    for i in range(len(eleName)):
        if d3.values[i][j]<>0:
            ...
            SmallTimeQ.append(d1.values[i][j+1])
            BigTimeQ.append(d2.values[i][j+1])
...
x = np.array(SmallQ)*(-1)
x1=np.array(SmallTimeQ)
y = np.array(BigQ)*(-1)
y1 = np.array(BigTimeQ)
ab=np.array([SmallQ,BigQ])
ab1=np.array([SmallTimeQ,BigTimeQ])
ar=np.corrcoef(ab)
ar1=np.corrcoef(ab1)
r=ar1[1][0]
tval=r*np.sqrt(len(SmallQ)-2)/np.sqrt(1-r*r)
pvalue=t.pdf(tval,len(SmallQ)-2)
pvalue=1-t.cdf(tval,len(SmallQ)-2)
...
fmt=FuncFormatter(to_percent)
plt.gca().yaxis.set_major_formatter(fmt)
plt.savefig("SamplingPrediction.png",dpi=120)
plt.show()
