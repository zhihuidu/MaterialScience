...
if len(sys.argv) < 2:
	print "arg error:\nusage: \tpython CalculatePercentage.py original-input-file Noise-percentage"
	exit(0)
...
for loop in range(IteNum):
    ...
    for i in range(0,19):
    	for j in range(1,20):
#	for j in range(0,20): change it from 0 to 1 to keep the baseline unchanged. Oct.1,2019
                tmprandom=random.uniform(0,1)
		change=1.0+noise*tmprandom*np.sign(0.5-random.uniform(0,1))
		d.values[i][j]=d_bak.values[i][j]*change
#		print "before=",d_bak.values[i][j]," after=",d.values[i][j]," noise=", noise, " change=", change
...
print "-------------Begin-----------"
print "Noise Best Scheduling Result for Input Case:",fn1, "Noise=", noise
...
print "Scheduling Result with Noise Best  Policy:"
